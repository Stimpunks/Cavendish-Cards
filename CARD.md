# CARD.md — template for a new card

One card, one file. Copy this file, fill it in, delete the guidance, open a pull request. That's the whole process.
...
> - Art is human- and community-made. The image cue is a motif for an illustrator, not finished art — don't paste AI-generated images. Until a human piece arrives, the card carries a hand-written vector placeholder from `scripts/card-art.py`, marked as one on the face; add a motif there when you add a card.
> - No scoring, no rewards, no "right" card. If a card ranks, judges, or is earned, it doesn't belong in this deck.
> - Counter-deficit only. A card names a need or a state or a change to the environment. It never names a person as the problem.
> - **Not an AAC board.** A card earns its place if it names a hard-to-voice inner state (a feeling, a sensory experience) or a niche-construction need — *and* carries a reframe in its Notes. It is not for wants, objects, or actions a person could request ("juice," "go outside"). That is communication, and it belongs to a person's real AAC, which this deck sits alongside and never replaces. Quick test: "can't tell" (an experience) belongs; "hungry" (a request) does not. Why the line is there: [Cavendish Cards are not AAC](https://cavendish.space/not-aac.html).
...
<!-- A motif for the illustrator. One or two lines. Concrete and warm. Human/community art only. -->

`[ what the picture shows ]`

## Prompt

<!-- Optional. A gentle line someone can voice, or the person can read. An invitation, never a demand.
     Love Locutions have no prompt — they are spoken between people, not read off the card. Leave this as — for those. -->
...
<!-- This is the card's guidebook entry. Every card gets one; they are assembled into the deck's guidebook.
     Write in the third person, about the card — never about the person. Keep the same short arc:
       1. The metaphor — what the image is.
       2. What it names — the experience or need it points to, in the person's frame.
       3. How to hold it — one line for the facilitator, always environment-facing.
     Describe the card, not the person. A card is never a diagnosis, a trait, or a verdict on the person holding it.
...
<!-- Optional. Reflection questions shown on the website when this card is turned up,
     ...
     defaults: ask about the card and the environment, never diagnose the person;
...
<!-- Optional. Facilitator layer only. Which Stimpunks Pattern(s) this card instantiates,
     for the guidebook and facilitator — it maps the person's word to the designer's.
