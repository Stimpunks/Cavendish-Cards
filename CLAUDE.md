...
- **Hand-authored pages** (no rebuild, Netlify serves as-is): `web/index.html` is the **threshold** — three doors, deliberately thin on prose; `web/deck.html` is the card player (it was `index.html` until 2026-09-15); `web/rooms.html` is the room zoner. Their shared shell is `web/styles.css`; `app.js` drives the deck and `rooms.js` the zoner.
...
- **Paired constants kept in sync by hand:** `SCREENING` and `FRAMEWORKS`/`FRAMEWORKS_LEAD` appear in **both** `build-site.py` and `build-guidebook.py` and must match word-for-word (sync comment present). `SITE_NAV` in `build-site.py` is mirrored in **three** hand-authored breakbars — `web/index.html` (the threshold), `web/deck.html` (the deck), and `web/rooms.html` — keep all four in sync when adding a page. There is a parity check worth running after any nav edit: compare the hrefs in `SITE_NAV` against the `<details class="disclose sitenav">` menu in each of the three files; they must match in order. The Pattern-crosswalk slug registry lives in `build-guidebook.py`; `build-site.py` imports it.
