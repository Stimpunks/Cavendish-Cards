...
# The five hand-authored pages that carry the menu, and the SITE_NAV key each
# one is currently on. Their menu block is WRITTEN BY THIS SCRIPT, between the
# nav:start / nav:end markers -- everything else in those files is hand-authored
# and untouched.
#
# It used to be six copies kept in step by hand, which is a rule that works
# until the day it doesn't: the check for it compared hrefs, so it could tell
# you the links matched and nothing about whether the groups did. Now there is
# one copy, in SITE_NAV, and these are renderings of it.
#
# They are tracked files, so a nav change leaves them dirty in `git status`
# after a build -- that is the reminder to commit them, the same as the tracked
# generated Markdown. Netlify runs this script on deploy, so even an uncommitted
# one is correct in production; the commit is for the repo's sake, not the
# site's.
HAND_AUTHORED_NAV = {
    "index.html": "home",
    "deck.html": "deck",
...
_NAV_START = ('<!-- nav:start \u2014 generated by scripts/build-site.py from '
              'SITE_NAV; do not edit by hand -->')
_NAV_END = "<!-- nav:end -->"
...
def _write_hand_authored_navs(web):
    """Rewrite the menu block in each hand-authored page from SITE_NAV.

    Fatal rather than a warning if a marker is missing. Every other check here
    is non-fatal because a wrong word still ships a working site; this one is
    different -- a lost marker means the file silently keeps whatever menu it
    had, which is exactly the stale-copy failure this function exists to end.
    Better a build that stops and says so."""
    written = []
    for name, key in HAND_AUTHORED_NAV.items():
        path = web / name
        text = path.read_text(encoding="utf-8")
        m = re.search(r"([ \t]*)" + re.escape(_NAV_START) + r".*?"
                      + re.escape(_NAV_END), text, re.S)
        if not m:
            sys.exit(f"{name}: no {_NAV_START[:18]}...{_NAV_END} block. The menu "
                     f"there is hand-written and will go stale -- restore the "
                     f"markers around it.")
        indent = m.group(1)
        block = (f"{indent}{_NAV_START}\n"
                 + site_nav_pretty(key, indent)
                 + f"\n{indent}{_NAV_END}")
        if block != m.group(0):
