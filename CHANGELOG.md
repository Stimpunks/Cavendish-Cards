...
- **The home page is now a threshold, not the deck.** `/` opens **Cavendish Space** — three doors, above the fold, no scrolling to reach a tool: *Show how you feel* (the deck), *Set up a room* (the zoner), and *Read the idea* (the model, at stimpunks.org). The deck keeps everything it had and now lives at **[/deck.html](https://cavendish.app/deck.html)**, first and largest among the doors because it is the one with an audience already pointed at it.
...
- The page is also honest about a variation rather than smoothing it over: caves, campfires, and watering holes are the three primordial spaces, while **the library and the habitat are additions** that appear in the deck and the room zoner. Five places in the tools, three in the lineage.
