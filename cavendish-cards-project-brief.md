...
- **Kind words reframe:** self-affirmation on the web; now framed as **given to someone, or claimed for yourself** (never earned, never a reward) — no longer gift-only, following the all-ages self-use reframe. The per-card label is now "given or claimed" (was "given, not read"), and the guidebook one-liner, starter-deck blurb, and BACKS section were updated to match. The distinct card back is retained but reframed as marking an *affirmation — held or given*, not "a gift." **Logged for Helen's batch review** (self-affirmation framing is hers).
