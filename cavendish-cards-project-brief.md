...
- **Kind words reframe:** self-affirmation on the web; now framed as **given to someone, or claimed for yourself** (never earned, never a reward) — no longer gift-only, following the all-ages self-use reframe. The per-card label is now "given or claimed" (was "given, not read"), and the guidebook one-liner, starter-deck blurb, and BACKS section were updated to match. The distinct card back is retained but reframed: it now reads **"a kind word" / "you don't have to earn it"** (was "a gift" / "given, not earned"), stating the anti-behaviorist stance as permission rather than a verdict. Placeholder faces and the face template read "given or claimed." **Logged for Helen's batch review** (self-affirmation framing is hers).
- **Distinct card backs = realms shown outside the face-down flow only.** The shared back is a privacy feature: face-down cards must be indistinguishable, or a spread would leak its contents before the person consents to share. So a distinct back is only safe for a realm that isn't laid face-down — **Love Locutions** (given/claimed) and **Interaction** (worn, shown face-up). Interaction now has its own back (badge trio + *how open i am* / *right now — show it, flip it*), because the standard *how i feel, what i need* misdescribes a worn availability signal. The other five realms keep the shared back, by necessity. Community-heritage design (ANI/Autreat badges) → logged for Helen.
